...
export function syscoinRpcWalletServices(callRpc, utilityServices) {
    ...
    async function addMultiSigAddress(nRequired, keys, account) {
        ...
        return account ? await callRpc('addmultiSigAddress', [nRequired, keys, account]) : await callRpc('addmultisigaddress', [nRequired, keys]);
    }
...
export function getPercentageFromWalletInfo(walletInfo) {
    ...
    if (walletInfo.code === constants.ERROR_CODE_NO_CONNECTION) {
        return false;
    }
